 ...
 * consider using the {@link com.example.ros_droid.Publisher} class wrapper for streamlining publishing.
 * <p>
...
public class RosBridge {
    ...
    protected Map<String, FragmentManager> fragmentManagers = new HashMap<String, FragmentManager>();
    ...
    public void waitForConnection(){
        ...
        if(this.connectionListener.getIsConnected()){
            return; //done
        }

        synchronized(this){
            while(!this.connectionListener.getIsConnected()){
                try {
                    this.wait();
    ...
    public boolean hasConnected(){
        return this.connectionListener.getIsConnected();
    }
    ...
    public void closeConnection(){
        ...
        this.connectionListener.setIsConnected(false);
    }
    ...
    public void onConnect(Session session) {
        ...
        this.connectionListener.setIsConnected(true);
        synchronized(this) {
            this.notifyAll();
        }
    ...
    public void subscribe(SubscriptionRequestMsg request, RosListenDelegate delegate){
        ...
        if(this.session != null){
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try{
                        Future<Void> fut = session.getRemote().sendStringByFuture(subMsg);
     ...
     */
    public void advertise(final String topic, final String type){
        ...
        if(this.session != null){
            ...
            if(!advertised){
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try{
                            Future<Void> fut = session.getRemote().sendStringByFuture(adMsg);
                            fut.get(2, TimeUnit.SECONDS);
                        }catch (Throwable t){
     ...
     */
    public void publish(final String topic, final String type, Object msg){
        ...
        if(this.session != null){
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try{
                        Future<Void> fut = session.getRemote().sendStringByFuture(jsonMsgString);
                        fut.get(2, TimeUnit.SECONDS);
                    }catch (Throwable t){
     ...
     */
    public void publishJsonMsg(final String topic, final String type, String jsonMsg){
        ...
        if(this.session != null){
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try{
                        Future<Void> fut = session.getRemote().sendStringByFuture(fullMsg);
                        fut.get(2, TimeUnit.SECONDS);
                    }catch (Throwable t){
                        System.out.println("Error publishing to " + topic + " with message type: " + type);
                        t.printStackTrace();
    ...
    protected void processFragment(JsonNode node){
        ...
        String fullMsg = null;
        synchronized(this.fragmentManagers){
            manager = this.fragmentManagers.get(id);
            if(manager == null){
